...
async def get_budget_category(trip_id: int, expense_type_id: int, repo: TripBudgetCategoryRepository = Depends(get_trip_budget_category_repository)):
    cat = await repo.get_by_trip_and_expense_type(trip_id, expense_type_id)
    if not cat:
...
async def set_budget_for_category(
    trip_id: int,
    expense_type_id: int,
    category: TripBudgetCategoryUpdate,
    repo: TripBudgetCategoryRepository = Depends(get_trip_budget_category_repository)
):
    updated = await repo.set_budget_for_category(trip_id, expense_type_id, category)
    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Budget category not found")
    return updated
